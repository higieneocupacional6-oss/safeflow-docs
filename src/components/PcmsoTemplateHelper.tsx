...
const SECTIONS: { title: string; vars: { tag: string; desc: string }[] }[] = [
  ...
  },
  {
    ...
    vars: [
      ...
      { tag: "{{gestor_contrato}}", desc: "Gestor do contrato (alias: {{gestor_nome}})" },
      { tag: "{{fiscal_contrato}}", desc: "Fiscal do contrato (alias: {{fiscal_nome}})" },
      { tag: "{{preposto_empresa}}", desc: "Preposto da empresa (alias: {{preposto_nome}})" },
    ],
  },
  {
    title: "Revisões (loop)",
