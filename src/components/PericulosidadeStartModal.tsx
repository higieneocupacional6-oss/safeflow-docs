...
import { cloneLtcatToTipo } from "@/lib/cloneLtcatToTipo";

type Mode = "choose" | "reaproveitar" | "zero";

export function PericulosidadeStartModal({
  open,
  onOpenChange,
}: {
  open: boolean;
  ...
  const { data: empresas = [] } = useQuery({
    queryKey: ["empresas-peric-modal"],
    queryFn: async () => {
      const { data, error } = await supabase.from("empresas")
        .select("id,razao_social,nome_fantasia").order("razao_social");
  ...
  const { data: ltcats = [], isLoading: loadingLtcats } = useQuery({
    queryKey: ["ltcats-empresa-peric", empresaId],
    queryFn: async () => {
      if (!empresaId) return [];
  ...
  const reset = () => {
    setMode("choose"); setEmpresaId(""); setLtcatId(""); setCloning(false);
  };
  const handleClose = (v: boolean) => { if (!v) reset(); onOpenChange(v); };

  const handleStartFromScratch = () => {
    handleClose(false);
    navigate("/documentos/periculosidade/novo");
  };

  const handleReaproveitar = async () => {
    if (!ltcatId) { toast.error("Selecione um LTCAT para reaproveitar"); return; }
    setCloning(true);
    try {
      const newId = await cloneLtcatToTipo(ltcatId, "periculosidade");
      console.log("DADOS COPIADOS DO LTCAT:", { ltcatId, newId });
      toast.success("Dados do LTCAT reaproveitados!");
      handleClose(false);
      navigate(`/documentos/periculosidade/editar/${newId}`);
    } catch (e: any) {
      ...
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          <DialogTitle className="font-heading">
            {mode === "choose" ? "Deseja reaproveitar dados do LTCAT?" : "Laudo de Periculosidade"}
          </DialogTitle>
        </DialogHeader>
          ...
          <div className="space-y-2 py-2">
            ...
            >
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-0.5">
                  Copia setores, funções e riscos qualitativos. Não copia medições/avaliações quantitativas.
                </p>
              </div>
            </button>
            <button
            ...
            >
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-0.5">
                  Cria um novo Laudo de Periculosidade em branco.
                </p>
              </div>
            </button>
          </div>
        ...
        {mode === "zero" && (
          <div className="py-2">
            <p className="text-sm text-muted-foreground mb-4">
              Você criará um novo Laudo de Periculosidade do zero (avaliação qualitativa, foco em risco de acidente).
            </p>
            <DialogFooter>
