...
import { ArrowLeft, Download, Edit2, Loader2, Plus, Trash2 } from "lucide-react";
...
import { FICHA_TIPOS, FichaTipo, fichaTipoLabel, formatFichaResultado, isFichaQuimica } from "@/lib/fichaTecnica";
import { baixarRelatorioFicha } from "@/lib/fichaTecnicaPdf";
...
export default function FichaTecnicaContrato() {
  ...
  return <div className="space-y-6">
    <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
      ...
      <div className="flex items-center gap-2"><Button variant="outline" onClick={() => baixarRelatorioFicha(contexto, resultados)} disabled={!resultados.length}><Download className="h-4 w-4 mr-2" />Baixar relatório</Button><DropdownMenu><DropdownMenuTrigger asChild><Button><Plus className="h-4 w-4 mr-2" />Resultados</Button></DropdownMenuTrigger><DropdownMenuContent align="end" className="w-72">{FICHA_TIPOS.map((item) => <DropdownMenuItem key={item.value} onClick={() => openNew(item.value)}>{item.label}</DropdownMenuItem>)}</DropdownMenuContent></DropdownMenu></div>
    </div>
